...
def Testing_function(k:int):
    ...
    for a in range(10):
        distances.append(L1_Norm(image, c1[a]))
        labels.append(1)

    for a in range(10):
        distances.append(L1_Norm(image, c2[a]))
        labels.append(2)
...
def L1_Norm(img_train, img_test):
    d = 0.0
    for a in range(len(img_train)):
        Manhattan_Ditance = d + (img_test[a] - img_train[a])

    return Manhattan_Ditance
